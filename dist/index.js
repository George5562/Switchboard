...
var SuiteConfigSchema = external_exports.object({
  suiteName: external_exports.string().optional(),
  description: external_exports.string().optional(),
  expose: external_exports.object({
  ...
  summaryMaxChars: external_exports.number().optional()
});
var ConfigSchema = external_exports.object({
  discoverGlobs: external_exports.array(external_exports.string()).default([".switchboard/mcps/*/.mcp.json"]),
  suites: external_exports.record(external_exports.string(), SuiteConfigSchema).default({}),
  timeouts: external_exports.object({
    childSpawnMs: external_exports.number().default(8e3),
    rpcMs: external_exports.number().default(6e4)
  }).default({
    childSpawnMs: 8e3,
    rpcMs: 6e4
  }),
  introspection: external_exports.object({
    mode: external_exports.enum(["summary", "full", "redacted"]).default("summary"),
    summaryMaxChars: external_exports.number().default(160)
  }).default({
    mode: "summary",
    summaryMaxChars: 160
  })
...
var defaultConfig = {
  ...
  suites: {},
  timeouts: { childSpawnMs: 8e3, rpcMs: 6e4 },
...
var ChildClient = class {
  constructor(meta, rpcTimeoutMs = 6e4) {
    this.meta = meta;
...
// src/core/router.ts
var childClients = /* @__PURE__ */ new Map();
async function listTopLevelTools(config) {
  ...
  for (const [childName, meta] of Object.entries(registry)) {
    ...
    const description = meta.switchboardDescription || suiteConfig?.description || `Use this tool for ${meta.description || childName}. Actions: 'introspect' | 'call'`;
    tools.push({
      name: suiteName,
      description,
      inputSchema: {
...
async function handleSuiteCall(toolName, params, config) {
...
// src/cli/init.ts
import { mkdir, writeFile, existsSync as existsSync2, readFile, rename } from "fs";
...
async function loadMcpDescriptions() {
  try {
    ...
    const descriptionsPath = join5(projectRoot, "mcp-descriptions-library.json");
...
async function getLibraryDescriptions() {
  ...
  try {
    ...
    const possiblePaths = [
      ...
      join2(currentDir, "..", "..", "mcp-descriptions-library.json"),
...
async function listMcpDirectories(mcpsDir) {
  ...
  for (const entry of entries) {
    ...
    const configPath = join2(mcpsDir, entry.name, ".mcp.json");
...
async function enableClaudeMode(mcpsDir, mcpNames) {
  ...
  for (const name of mcpNames) {
    ...
    const originalPath = join2(mcpDir, ".mcp.json");
    const archivedDir = join2(mcpDir, "original");
    const archivedPath = join2(archivedDir, ".mcp.json");
    if (!existsSync2(originalPath) && existsSync2(archivedPath)) {
      wrapped.push(name);
      continue;
    }
...
async function copyExistingMcps(existingConfig, switchboardDir) {
  ...
  for (const [mcpName, mcpConfig] of Object.entries(mcpsSection)) {
    if (mcpName === "switchboard") continue;
    const mcpDir = join2(mcpsDir, mcpName);
    ...
    await writeFileAsync(join2(mcpDir, ".mcp.json"), JSON.stringify(mcpJsonContent, null, 2));
...
async function initSwitchboard(cwd) {
  ...
  try {
    ...
    const { copiedMcps, libraryDescriptions } = existingConfig ? await copyExistingMcps(existingConfig, switchboardDir) : { copiedMcps: [], libraryDescriptions: [] };
    if (copiedMcps.length === 0) {
      const exampleMcpDir = join2(mcpsDir, "example-mcp");
      await mkdirAsync(exampleMcpDir, { recursive: true });
      await writeFileAsync(join2(exampleMcpDir, ".mcp.json"), TEMPLATE_MCP_JSON);
    }
...
async function getLibraryDescription(mcpName) {
  try {
    const descPath = join4(process.cwd(), "mcp-descriptions-library.json");
...
async function addMcpToSwitchboard(cwd, args) {
  ...
  if (!description) {
    ...
    if (libraryDesc) {
      ...
      const useLibrary = await promptYesNo2("Use this description?", true);
      if (useLibrary) {
...
`);
  ...
  const server = new McpServer({
    name: "switchboard",
    version: "0.1.0",
    // This could be dynamically loaded from package.json
    capabilities: {
      tools: {}
    }
  });
  ...
  for (const tool of tools) {
    const toolSchema = {
      action: external_exports.enum(["introspect", "call"]),
      subtool: external_exports.string().optional(),
      args: external_exports.record(external_exports.string(), external_exports.any()).optional()
    };
    server.tool(tool.name, tool.description, toolSchema, async (args2, _extra) => {
      const result = await handleSuiteCall(tool.name, args2, config);
      return {
