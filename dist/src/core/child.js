...
export class ClaudeChildClient extends ChildClient {
    ...
    constructor(meta, rpcTimeoutMs = 60000, idleTimeoutMs = 300000) {
    ...
    checkIdle() {
        ...
        if (idleTime > this.idleTimeoutMs) {
            process.stderr.write(`[ClaudeChildClient] Idle timeout (${Math.round(idleTime / 1000)}s) for ${this.meta.name}, shutting down gracefully...\n`);
    ...
    async gracefulShutdown() {
        ...
        if (this.process) {
            process.stderr.write(`[ClaudeChildClient] Sending SIGTERM to ${this.meta.name}...\n`);
