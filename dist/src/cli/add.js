...
async function setupClaudeServer(mcpDir, mcpName, originalConfig) {
    ...
    await writeFileAsync(join(mcpDir, '.mcp.json'), JSON.stringify(childMcpConfig, null, 2));
    // Create CLAUDE.md
...
export async function addMcpToSwitchboard(cwd, args) {
    ...
    if (!description) {
        ...
        if (!description) {
            description = await promptForInput('Description for LLM (one line)', `Describe what ${mcpName} does`);
        }
    }
    ...
    if (options.claudeServer) {
        ...
        await writeFileAsync(join(originalDir, '.mcp.json'), JSON.stringify(config, null, 2));
